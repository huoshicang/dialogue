...
import { generateRandomString } from "@/tools/randomStringGenerator.js";

// 在每次路由切换之前执行的函数

router.beforeEach(async (to, from, next): Promise<void> => {
  const loginUser = useUserStore().user_info;
  ...
  if (needAccess !== ACCESS_ENUM.NOT_LOGIN) {
    ...
    if (loginUser.name === "未登录" || !loginUser.role) {
      next(`/login/${localStorage.getItem("log") ?? generateRandomString()}`);
      return;
    }
  }
